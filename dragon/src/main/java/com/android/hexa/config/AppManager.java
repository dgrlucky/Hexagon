...
import static com.android.hexa.config.Platform.DEPENDENCY_SUPPORT_DESIGN;
...
public final class AppManager {
        ...
        Completable.fromAction(new Action() {
            ...
            public void run() throws Exception {
                ...
                if (DEPENDENCY_SUPPORT_DESIGN) {
                    ...
                    HexaUtil.makeText(mApplication, message);
                }
            }
     ...
     * 如果在 AppInject 启动第一个 {@link Activity} 执行 {@link Activity#onCreate} 方法时使用 {@link #getCurrentActivity()} 则会出现返回为 {@code null} 的情况
     ...
     * 是在 {@link Activity#onStop()} 之前, 所以如果当此 {@link Activity} 调用 {@link Activity#onStop()} 方法之后, 没有其他的 {@link Activity} 回到前台(用户返回桌面或者打开了其他 AppInject 会出现此状况)
     ...
     * 如果返回为 {@code null}, 说明没有前台 {@link Activity} (用户返回桌面或者打开了其他 AppInject 会出现此状况), 则不做任何操作, 不为 {@code null}, 则弹出 {@link Dialog}
     *
     * @return
     */
    @Nullable
     ...
     * 即使 AppInject 进入后台或在这个 {@link Activity} 中打开一个之前已经存在的 {@link Activity}, 这时调用此方法
...
//        }
     ...
     * 退出应用程序
     * <p>
     * 此方法经测试在某些机型上并不能完全杀死 AppInject 进程, 几乎试过市面上大部分杀死进程的方式, 但都发现没卵用, 所以此
