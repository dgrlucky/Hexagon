...
import com.android.hexa.di.component.DaggerAppComponent;
import com.android.hexa.di.module.GlobalConfigModule;
import com.android.hexa.config.ConfigModule;
import com.android.hexa.config.ManifestParser;
 ...
 * 这时就不用再继承 HexaApplication,只用在自定义Application中对应的生命周期调用AppDelegate对应的方法
 * (Application一定要实现APP接口),框架就能照常运行
 */
public class AppDelegate implements AppInject, AppLifecycles {
    ...
    public void onCreate(@NonNull Application application) {
        ...
        //执行框架外部, 开发者扩展的 AppInject onCreate 逻辑
        for (AppLifecycles lifecycle : mAppLifecycles) {
    ...
    public AppComponent getAppComponent() {
        Preconditions.checkNotNull(mAppComponent, "%s == null, first call %s#onCreate(Application) in %s#onCreate()",
     ...
     * 响应 {@link ComponentCallbacks2#onTrimMemory(int)} 回调, 开发者的 AppInject 会存活的更持久, 有利于用户体验
    ...
    private static class AppComponentCallbacks implements ComponentCallbacks2 {
        ...
        /**
         * 在你的 AppInject 生命周期的任何阶段, {@link ComponentCallbacks2#onTrimMemory(int)} 发生的回调都预示着你设备的内存资源已经开始紧张
        ...
        public void onTrimMemory(int level) {
            //状态1. 当开发者的 AppInject 正在运行
            //设备开始运行缓慢, 不会被 kill, 也不会被列为可杀死的, 但是设备此时正运行于低内存状态下, 系统开始触发杀死 LRU 列表中的进程的机制
//                case TRIM_MEMORY_RUNNING_MODERATE:


            //设备运行更缓慢了, 不会被 kill, 但请你回收 unused 资源, 以便提升系统的性能, 你应该释放不用的资源用来提升系统性能 (但是这也会直接影响到你的 AppInject 的性能)
//                case TRIM_MEMORY_RUNNING_LOW:


            //设备运行特别慢, 当前 AppInject 还不会被杀死, 但是系统已经把 LRU 列表中的大多数进程都已经杀死, 因此你应该立即释放所有非必须的资源
            //如果系统不能回收到足够的 RAM 数量, 系统将会清除所有的 LRU 列表中的进程, 并且开始杀死那些之前被认为不应该杀死的进程, 例如那个包含了一个运行态 Service 的进程
//                case TRIM_MEMORY_RUNNING_CRITICAL:


            //状态2. 当前 AppInject UI 不再可见, 这是一个回收大个资源的好时机
//                case TRIM_MEMORY_UI_HIDDEN:


            //状态3. 当前的 AppInject 进程被置于 Background LRU 列表中
            //进程位于 LRU 列表的上端, 尽管你的 AppInject 进程并不是处于被杀掉的高危险状态, 但系统可能已经开始杀掉 LRU 列表中的其他进程了
            //你应该释放那些容易恢复的资源, 以便于你的进程可以保留下来, 这样当用户回退到你的 AppInject 的时候才能够迅速恢复
...
//                case TRIM_MEMORY_MODERATE:
            ...
            //系统正运行于低内存的状态并且你的进程正处于 LRU 列表中最容易被杀掉的位置, 你应该释放任何不影响你的 AppInject 恢复状态的资源
            //低于 API 14 的 AppInject 可以使用 onLowMemory 回调
